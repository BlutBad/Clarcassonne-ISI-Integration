...
/* //Carga los nombre de los participantes de una partida
Template.roomplayerstemp.players=function(){
	var players = [];
	// HAY QUE PELEAR CON ESTO
	var n = Plays.find({match_id : Session.get('match_id')}).count();
	var plays_list = Plays.find({match_id : Session.get('match_id')});
	for(var i = 0; i < n; i++){
		players.push({score : plays_list[i].score});
		console.log({score : plays_list[i].score});
	}
	return players;
} */
...
Template.matchestemp.events = {
	...
	'click a.linkmatch':function(event){
		...
		if(Plays.find({match_id : $(this)[0]._id}).count() < lim || Plays.findOne({match_id : $(this)[0]._id, user_id : Meteor.userId()})){
			...
			if (Games.findOne({_id : Session.get('game_id')}).name=="Alien_Invasion")
				...
				$('#frootwarscontainer').show();
			if(!Plays.findOne({match_id : Session.get('match_id'), user_id : Meteor.userId()})){
				console.log(Plays.findOne({match_id : Session.get('match_id'), user_id : Meteor.userId()}));
				Matches.update({_id : Session.get('match_id')}, {$inc: {num_players: 1}});
				Plays.insert({match_id : Session.get('match_id'), user_id : Meteor.userId(), score : 0});
			}
		} else {
